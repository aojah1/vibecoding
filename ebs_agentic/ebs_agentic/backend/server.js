...
// Execute OCI GenAI prompt via Python bridge
async function executeOCI({ systemPrompt = '', userPrompt = '', maxTokens = 12000, temperature = 0.2 }) {
  const pythonFromVenv = path.join(__dirname, '.oci_venv', 'bin', 'python');
  const bridgeScript = path.join(__dirname, 'oci_genai_bridge.py');

  return new Promise((resolve, reject) => {
    const child = spawn(pythonFromVenv, [bridgeScript], {
      cwd: __dirname,
      env: process.env
    });
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        return reject(new Error(stderr || `OCI bridge exited with code ${code}`));
      }

      try {
        const parsed = JSON.parse(stdout || '{}');
        if (!parsed.success) {
          return reject(new Error(parsed.error || 'OCI bridge returned unsuccessful response'));
        }
        resolve(parsed.text || '');
      } catch (e) {
        reject(new Error(`OCI bridge invalid JSON output: ${stdout}`));
      }
    });
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    console.log('🤖 Generating SQL with OCI GenAI...');
...
Respond with ONLY the SQL query.`;
    ...
    const sqlText = await executeOCI({
      systemPrompt,
      userPrompt: question,
      maxTokens: 2048,
      temperature: 0.1
    ...
    let sql = (sqlText || '').trim().replace(/```sql\n?/g, '').replace(/```\n?/g, '').trim();
...
Provide a clear, concise answer.`;
    ...
    const answerText = await executeOCI({
      systemPrompt: `You are an Oracle EBS Accounts Payable analyst. CRITICAL: Today's date is ${ANALYSIS_DATE} (October 10, 2010). All aging calculations, urgency assessments, and date references must be relative to this date. Do NOT reference any date after October 2010.`,
...
app.post('/api/analyze-supplier', async (req, res) => {
  try {
    ...
    console.log('🤖 Generating AI analysis with OCI GenAI...');
...
app.post('/api/analyze-supplier-stream', async (req, res) => {
  try {
    ...
    // Step 5: Call OCI GenAI
    sendEvent('thought', { message: '🤖 Initiating AI analysis with OCI GenAI...' });
...
Show your reasoning process clearly.`;
    ...
    sendEvent('thought', { message: '💭 OCI GenAI is analyzing the data...' });

    const fullInsights = await executeOCI({
      systemPrompt: `You are an Oracle EBS Accounts Payable analyst. CRITICAL: Today's date is ${ANALYSIS_DATE} (October 10, 2010). All aging calculations, urgency assessments, and date references must be relative to this date. Do NOT reference any date after October 2010. When you see "X days old", that means X days before ${ANALYSIS_DATE}.`,
      userPrompt: analysisPrompt,
      maxTokens: 12000,
      temperature: 0.2
    ...
    // Pseudo-stream chunks to keep frontend SSE behavior
    const words = (fullInsights || '').split(/(\s+)/);
    let buffer = '';
    for (const token of words) {
      buffer += token;
      if (buffer.length >= 120) {
        sendEvent('ai-stream', { text: buffer });
        buffer = '';
      }
    }
    if (buffer) {
