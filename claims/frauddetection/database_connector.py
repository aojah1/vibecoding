...
class DatabaseConnector:
    ...
    def transform_db_record_to_claim(self, record: Dict) -> Dict:
        ...
        claim = {
            ...
            'estimated_loss': float((record.get('ESTIMATED_LOSS', record.get('CLAIM_AMOUNT', record.get('AMOUNT', record.get('TOTAL_AMOUNT', 0)))) or 0)),
            'ESTIMATED_LOSS': float((record.get('ESTIMATED_LOSS', record.get('CLAIM_AMOUNT', record.get('AMOUNT', record.get('TOTAL_AMOUNT', 0)))) or 0)),
            'claim_amount': float((record.get('ESTIMATED_LOSS', record.get('CLAIM_AMOUNT', record.get('AMOUNT', record.get('TOTAL_AMOUNT', 0)))) or 0)),
