...
def analyze_claim():
    ...
    try:
        ...
        # Normalize amount field from DB/clients
        if 'estimated_loss' not in claim_data and 'ESTIMATED_LOSS' in claim_data:
            claim_data['estimated_loss'] = claim_data['ESTIMATED_LOSS']
        # Validate required fields
        required_fields = ['claim_id', 'claim_type', 'estimated_loss']
...
def batch_analyze():
    ...
    try:
        ...
        results = []
        for claim in claims:
            # Normalize amount key for analyzer
            if 'estimated_loss' not in claim and 'ESTIMATED_LOSS' in claim:
                claim['estimated_loss'] = claim['ESTIMATED_LOSS']
    ...
    try:
        ...
        for i in range(count):
            ...
            claim = {
                ...
                'ESTIMATED_LOSS': random.randint(5000, 50000),
            ...
            # Backward/forward compatibility: set lowercase key used by analyzer
            claim['estimated_loss'] = claim['ESTIMATED_LOSS']
            ...
            try:
                ...
                for i, record in enumerate(db_records):
                    try:
                        ...
                        yield f"data: {json.dumps({'type': 'claim_loaded', 'claim_id': claim.get('claim_id'), 'claim_type': claim.get('claim_type'), 'amount': claim.get('ESTIMATED_LOSS'), 'current': i+1, 'total': len(db_records)})}\n\n"
                    except Exception as e:
...
def generate_sample_claims_from_db(count: int = 10) -> List[Dict]:
    ...
    for i in range(count):
        ...
        claim = {
            ...
            'CLAIM_TYPE': random.choice(claim_types),
            'ESTIMATED_LOSS': random.randint(5000, 75000),
            'INCIDENT_DATE': incident_date,
            'FILING_DATE': filing_date,
