...
export function setupChatbotEndpoints(app) {
  ...
  // --------------------------------------------------------------------------
  // POST /api/claims/auto-approve - Auto-Approve Agent
  // Approves all claims where ASSIGNED_ADJUSTER_ID = 'AUTO_APPROVE'
  // --------------------------------------------------------------------------
  app.post('/api/claims/auto-approve', async (req, res) => {
    console.log('\n╔════════════════════════════════════════╗');
    console.log('║  🤖 AUTO-APPROVE AGENT TRIGGERED      ║');
    console.log('╚════════════════════════════════════════╝');

    try {
      // Step 1: Find eligible claims
      const findSql = `
        SELECT CLAIM_ID, CLAIM_NUMBER, STATUS, ESTIMATED_LOSS
        FROM CLAIMS
        WHERE ASSIGNED_ADJUSTER_ID = 'AUTO_APPROVE'
          AND UPPER(STATUS) != 'APPROVED'
      `;
      const findResult = await run(findSql);
      const eligible = findResult?.rows || [];

      console.log(`[Auto-Approve] Found ${eligible.length} eligible claims`);

      if (eligible.length === 0) {
        return res.json({
          success: true,
          message: 'No eligible claims found for auto-approval.',
          approvedCount: 0,
          claims: []
        });
      }

      // Step 2: Update all eligible claims to Approved
      const updateSql = `
        UPDATE CLAIMS
        SET STATUS = 'Approved',
        ...
        WHERE ASSIGNED_ADJUSTER_ID = 'AUTO_APPROVE'
          AND UPPER(STATUS) != 'APPROVED'
      `;
      const updateResult = await run(updateSql);
      const rowsAffected = updateResult?.rowsAffected || 0;

      console.log(`[Auto-Approve] ✅ Approved ${rowsAffected} claims`);
      console.log('════════════════════════════════════════\n');

      return res.json({
        success: true,
        message: `Auto-Approve Agent successfully approved ${rowsAffected} claim(s).`,
        approvedCount: rowsAffected,
      ...
      return res.status(500).json({
        success: false,
        message: 'Auto-Approve Agent failed: ' + error.message
      });
    }
  });
  ...
  console.log('   POST   /api/claims/auto-approve');
  console.log('════════════════════════════════════════\n');
}
